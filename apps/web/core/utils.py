...
def build_recipes_dict(all_rows):
    ...
    for row in all_rows:
        ...
        if (
            ingredient_id
            and ingredient_id not in recipes_dict[recipe_id]["ingredients"]
        ):
            ...
            if quantity_g > 0:
                recipes_dict[recipe_id]["calculated_nutrition"]["energy"] += (
                    ing_nutrition["energy"]
                )
                recipes_dict[recipe_id]["calculated_nutrition"]["protein"] += (
                    ing_nutrition["protein"]
                )
                recipes_dict[recipe_id]["calculated_nutrition"]["fat"] += ing_nutrition[
                    "fat"
                ]
                recipes_dict[recipe_id]["calculated_nutrition"]["carbs"] += (
                    ing_nutrition["carbs"]
                )
        ...
        if (
            ...
            and row["position"] not in recipes_dict[recipe_id]["steps"]
        ):
            recipes_dict[recipe_id]["steps"][row["position"]] = {
                "memo": row["step_memo"]
            }
    return recipes_dict
