    ...
    if inclusions:
        ...
        if len(inclusions) == 1:
            ...
            for group in inclusions:
                ...
                for syn in group:
                    cursor.execute(
                        "SELECT count(*) as cnt FROM ingredients WHERE name = %s",
                        (syn,),
                    )
                    row = cursor.fetchone()
                    if row:
...
def search_standard_recipes(cursor, search_query, search_mode="recipe"):
    ...
    if search_mode == "ingredient":
        ...
        if raw_inclusions:
            for keyword in raw_inclusions:
                ...
                matches = {
                    row["standard_recipe_id"]: row["count"] for row in cursor.fetchall()
                }
            ...
            if exclusions:
                ...
                scored_recipes = [
                    r for r in scored_recipes if r["id"] not in excluded_ids
                ]
    ...
    for r_id in recipes_data:
        sorted_ingredients = dict(
            sorted(
                recipes_data[r_id]["ingredient"].items(),
                key=lambda item: item[1]["all"][0],
                reverse=True,
            )
        )
...
def get_standard_recipe_details(cursor, recipe_id):
    ...
    sorted_ingredients_data = dict(
        sorted(
            ingredients_data.items(), key=lambda item: item[1]["all"][0], reverse=True
        )
    )
    recipe["ingredient"] = (
        sorted_ingredients_data  # Use 'ingredient' key to match search results template
    )
