...
def get_synonyms(cursor: Any, keyword: str) -> list[str]:
    ...
    sql_get_normalized = (
        "SELECT normalized_name FROM synonym_dictionary WHERE synonym = %s"
    )
...
def verify_search(search_query: str) -> None:
    ...
    if inclusions:
        ...
        for syn_group in inclusions:
            ...
            conditions.append(
                f"SUM(CASE WHEN i.name IN ({placeholders}) THEN 1 ELSE 0 END) > 0"
            )
        ...
        final_candidates = [
            row for row in candidates if row["attribute"] == selected_attribute
        ]
